...
public class SparkIOExceptionHandlerChain {
  ...
  public static SparkIOExceptionHandlerChain getSparkIOExceptionHandlerChain(
      JobConf conf) {
    SparkIOExceptionHandlerChain chain = new SparkIOExceptionHandlerChain();
    String exceptionHandlerStr = conf.get(SPARK_IO_EXCEPTION_HANDLE_CHAIN);
    List<SparkIOExceptionHandler> handlerChain = new ArrayList<SparkIOExceptionHandler>();
      ...
      if (handlerArr != null && handlerArr.length > 0) {
        for (String handlerStr : handlerArr) {
          if (!handlerStr.trim().equals("")) {
            try {
              Class<? extends SparkIOExceptionHandler> handlerCls =
                (Class<? extends SparkIOExceptionHandler>) Class.forName(handlerStr);
  ...
  private List<SparkIOExceptionHandler> handlerChain;

  /**
   * @return the exception handler chain defined
   */
  protected List<SparkIOExceptionHandler> getHandlerChain() {
    return handlerChain;
  }
   ...
   * @param handlerChain
   */
  protected void setHandlerChain(List<SparkIOExceptionHandler> handlerChain) {
  ...
  public RecordReader<?,?> handleRecordReaderCreationException(Exception e) throws IOException {
    ...
    if (handlerChain != null && handlerChain.size() > 0) {
      for (SparkIOExceptionHandler handler : handlerChain) {
   ...
   * SparkIOExceptionNextHandleResult to store the results of each handler. If
   * the exception is handled by one handler, the handler should set
   * SparkIOExceptionNextHandleResult to be handled, and also set the handle
  ...
  public boolean handleRecordReaderNextException(Exception e)
      throws IOException {
    SparkIOExceptionNextHandleResult result = new SparkIOExceptionNextHandleResult();
    if (handlerChain != null && handlerChain.size() > 0) {
      for (SparkIOExceptionHandler handler : handlerChain) {
