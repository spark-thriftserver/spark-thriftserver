...
public class ZooKeeperTokenStore implements DelegationTokenStore {
  ...
  private final String WHEN_ZK_DSTORE_MSG = "when zookeeper based delegation token storage is enabled"
      + "(spark.cluster.delegation.token.store.class=" + ZooKeeperTokenStore.class.getName() + ")";
  ...
  private void setupJAASConfig(Configuration conf) throws IOException {
    ...
    String principal = SQLConf.get().getConfString(ServiceConf.THRIFTSERVER_KERBEROS_PRINCIPAL().key());
    String keytab = SQLConf.get().getConfString(ServiceConf.THRIFTSERVER_KERBEROS_KEYTAB().key());
  ...
  public void init(Object hmsHandler, HadoopThriftAuthBridge.Server.ServerMode smode) {
    ...
    zkConnectString =
        conf.get(SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_STR, null);
    if (zkConnectString == null || zkConnectString.trim().isEmpty()) {
      // try alternate config param
      zkConnectString =
          conf.get(
              SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_STR_ALTERNATE,
              null);
      if (zkConnectString == null || zkConnectString.trim().isEmpty()) {
        throw new IllegalArgumentException("Zookeeper connect string has to be specifed through "
            + "either " + SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_STR
            + " or "
            + SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_STR_ALTERNATE
            + WHEN_ZK_DSTORE_MSG);
      }
    ...
    connectTimeoutMillis =
        conf.getInt(
            SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_CONNECT_TIMEOUTMILLIS,
            CuratorFrameworkFactory.builder().getConnectionTimeoutMs());
    String aclStr = conf.get(SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ACL, null);
    if (StringUtils.isNotBlank(aclStr)) {
      this.newNodeAcl = parseACLs(aclStr);
    }
    rootNode =
        conf.get(SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ZNODE,
            SparkDelegationTokenManager.DELEGATION_TOKEN_STORE_ZK_ZNODE_DEFAULT) + serverMode;
