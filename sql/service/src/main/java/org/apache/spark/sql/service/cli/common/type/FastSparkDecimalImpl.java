...
/**
 *    This class is a companion to the FastSparkDecimal class that separates the essential of code
 *    out of FastSparkDecimal into static methods in this class so that they can be used directly
 ...
 */
public class FastSparkDecimalImpl extends FastSparkDecimal {
  ...
  public static boolean fastSetFromBytes(byte[] bytes, int offset, int length, boolean trimBlanks,
    ...
    while (true) {
      ...
      if (digitNum == 0) {
        ...
        if (longWordIndex == 0) {
          ...
          // We have filled SparkDecimal.MAX_PRECISION digits and have no more room in our limit precision
          // fast decimal.
          return false;
        }
        longWordIndex++;
    ...
    if (index >= end) {
      ...
      final int scaleDown = SparkDecimal.MAX_PRECISION - precision;
      if (scaleDown > 0) {
    ...
    if (integerDigitCount == 0 && nonTrailingZeroScale == 0) {
      ...
      final int scaleDown = SparkDecimal.MAX_PRECISION - precision + trailingZeroCount;
      if (scaleDown > 0) {
        doFastScaleDown(fast0, fast1, fast2, scaleDown, fastResult);
      } else {
        fastResult.fast0 = fast0;
    ...
    if (roundingNecessary) {
      ...
      if (fastResult.fastSignum == 0) {
        ...
        fastResult.fastScale = SparkDecimal.MAX_SCALE;
      } else {
        if (!fastAdd(
  ...
  public static boolean fastSetFromBigDecimal(
          BigDecimal bigDecimal, boolean allowRounding, FastSparkDecimal fastResult) {
    ...
    if (!allowRounding) {
      if (scale < 0 || scale > SparkDecimal.MAX_SCALE) {
        return false;
      }
  ...
  public static boolean fastSetFromString(
          String string, boolean trimBlanks, FastSparkDecimal result) {
   ...
   * @param fastResult an object to reuse
   */
  public static void fastSetFromInt(int intValue, FastSparkDecimal fastResult) {
    if (intValue == 0) {
      // Zero special case.
      return;
    }
  ...
  public static void fastSetFromLong(
          long longValue, FastSparkDecimal fastResult) {
    if (longValue == 0) {
      // Zero special case.
      return;
    }
  ...
  public static boolean fastSetFromLongAndScale(
          long longValue, int scale, FastSparkDecimal fastResult) {
  ...
  public static boolean fastSetFromFloat(
          float floatValue, FastSparkDecimal fastResult) {
  ...
  public static boolean fastSetFromDouble(
          double doubleValue, FastSparkDecimal fastResult) {
   ...
   * @param fastResult an object to reuse
   * @return Return true if the BigInteger value fit within SparkDecimal.MAX_PRECISION.  Otherwise,
   *         false for overflow.
   */
  public static boolean fastSetFromBigInteger(
          BigInteger bigInteger, FastSparkDecimal fastResult) {
   ...
   */
  public static boolean fastSetFromBigInteger(
          BigInteger bigInteger, int scale, FastSparkDecimal fastResult) {
    ...
    if (integerDigitCount >= 0) {
      if (integerDigitCount > SparkDecimal.MAX_PRECISION) {
        return false;
      }
      maxScale = SparkDecimal.MAX_SCALE - integerDigitCount;
    } else {
      maxScale = SparkDecimal.MAX_SCALE;
    }
  ...
  public static boolean doBinaryToDecimalConversion(
          ...
          FastSparkDecimal middleWordMultiplier,
          FastSparkDecimal highWordMultiplier,
          FastSparkDecimal fastResult) {
   ...
   * SparkDecimal needs 128 bits of binary which Java does not provide primitive support for)
  ...
  public static boolean doDecimalToBinaryDivisionRemainder(
          ...
          FastSparkDecimal fastInverseConst,
          int quotientIntegerWordNum,
          int quotientIntegerDigitNum,
          FastSparkDecimal fastMultiplierConst,
          long[] scratchLongs) {
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_62 =
          new FastSparkDecimal(1, 1686018427387904L, 461L, 0, 19, 0);
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_63 =
          new FastSparkDecimal(1, 3372036854775808L, 922L, 0, 19, 0);
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_125 =
          new FastSparkDecimal(1, 1825928971026432L, 9586511730793292L, 425352L, 38, 0);
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_63_INVERSE =
          new FastSparkDecimal(1, 6994171142578125L, 5044340074528008L, 1084202172485L, 45, 0);
  ...
  public static boolean fastSerializationUtilsRead(InputStream inputStream, int scale,
                                                   byte[] scratchBytes,
                                                   FastSparkDecimal fastResult) throws IOException {
  ...
  public static boolean fastSerializationUtilsWrite(OutputStream outputStream,
     ...
     *   do not have to since FastSparkDecimal stores the numbers unsigned in fast0, fast1,
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_56 =
          new FastSparkDecimal(1, 2057594037927936L, 7L, 0, 17, 0);
   ...
   */
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_112 =
          new FastSparkDecimal(1, 8530496329220096L, 9229685853482762L, 51L, 34, 0);
  ...
  //
  private static final FastSparkDecimal FAST_HIVE_DECIMAL_TWO_POWER_56_INVERSE =
          new FastSparkDecimal(1, 9585113525390625L, 8078144567552953L, 13877787L, 40, 0);
  ...
  public static int fastBigIntegerBytesScaled(
    ...
    if (fastSignum == 0 || serializeScale == fastScale) {
      ...
      final int maxScale = SparkDecimal.MAX_SCALE - fastIntegerDigitCount;
      ...
      FastSparkDecimal fastTemp = new FastSparkDecimal();
      if (!fastScaleUp(
              fast0, fast1, fast2,
              scaleUp,
              fastTemp)) {
  ...
  private static final FastSparkDecimal FASTHIVEDECIMAL_MIN_BYTE_VALUE_MINUS_ONE =
          new FastSparkDecimal((long) Byte.MIN_VALUE - 1L);
  private static final FastSparkDecimal FASTHIVEDECIMAL_MAX_BYTE_VALUE_PLUS_ONE =
          new FastSparkDecimal((long) Byte.MAX_VALUE + 1L);

  private static final int MAX_SHORT_DIGITS = 5;
  private static final FastSparkDecimal FASTHIVEDECIMAL_MIN_SHORT_VALUE_MINUS_ONE =
          new FastSparkDecimal((long) Short.MIN_VALUE - 1L);
  private static final FastSparkDecimal FASTHIVEDECIMAL_MAX_SHORT_VALUE_PLUS_ONE =
          new FastSparkDecimal((long) Short.MAX_VALUE + 1L);

  private static final int MAX_INT_DIGITS = 10;
  private static final FastSparkDecimal FASTHIVEDECIMAL_MIN_INT_VALUE_MINUS_ONE =
          new FastSparkDecimal((long) Integer.MIN_VALUE - 1L);
  private static final FastSparkDecimal FASTHIVEDECIMAL_MAX_INT_VALUE_PLUS_ONE =
          new FastSparkDecimal((long) Integer.MAX_VALUE + 1L);

  private static final FastSparkDecimal FASTHIVEDECIMAL_MIN_LONG_VALUE =
          new FastSparkDecimal(Long.MIN_VALUE);
  private static final FastSparkDecimal FASTHIVEDECIMAL_MAX_LONG_VALUE =
          new FastSparkDecimal(Long.MAX_VALUE);
  private static final int MAX_LONG_DIGITS =
          FASTHIVEDECIMAL_MAX_LONG_VALUE.fastIntegerDigitCount;
  private static final FastSparkDecimal FASTHIVEDECIMAL_MIN_LONG_VALUE_MINUS_ONE =
          new FastSparkDecimal("-9223372036854775809");
  private static final FastSparkDecimal FASTHIVEDECIMAL_MAX_LONG_VALUE_PLUS_ONE =
          new FastSparkDecimal("9223372036854775808");
  ...
  public static boolean fastIsLong(
    ...
    if (fastIntegerDigitCount < MAX_LONG_DIGITS) {
      ...
      if (fastSignum == 1) {
        FastSparkDecimal max = FASTHIVEDECIMAL_MAX_LONG_VALUE;
        if (fast1 > max.fast1 || (fast1 == max.fast1 && fast0 > max.fast0)) {
          return false;
        }
        return true;
      } else {
        FastSparkDecimal min = FASTHIVEDECIMAL_MIN_LONG_VALUE;
  ...
  public static BigInteger fastBigIntegerValueScaled(
    ...
    //
    // Since we are emulating old behavior and recommending the use of SparkDecimal.bigIntegerBytesScaled
  ...
  public static int fastCompareTo(
    ...
    if (leftScale == rightScale) {
      ...
      // temporary FastSparkDecimal object.
  ...
  public static int fastHashCode(
    ...
    // verifies the OldHiveDecimal.bigDecimalValue() matches (new) SparkDecimal.bigDecimalValue().
  ...
  public static boolean fastScaleByPowerOfTen(
    ...
    if (power > 0) {
      ...
      if (fastIntegerDigitCount > 0) {
        ...
        integerRoom = SparkDecimal.MAX_PRECISION - fastIntegerDigitCount;
        ...
        //
        if (power > SparkDecimal.MAX_PRECISION + zeroesBelowDot) {
    ...
    } else if (fastScale + absPower <= SparkDecimal.MAX_SCALE) {
      ...
      // fastScale + absPower > SparkDecimal.MAX_SCALE

      // Look at getting rid of fractional digits that will now be below SparkDecimal.MAX_SCALE.

      final int scaleDown = fastScale + absPower - SparkDecimal.MAX_SCALE;

      if (scaleDown < SparkDecimal.MAX_SCALE) {
  ...
  public static boolean doFastRound(
    ...
    if (roundPower > fastScale) {
      ...
      // We pretend to add trailing zeroes, EVEN WHEN it would exceed the SparkDecimal.MAX_PRECISION.
  ...
  public static boolean fastRoundIntegerUp(
    ...
    if (roundingPoint > SparkDecimal.MAX_PRECISION) {
  ...
  public static boolean fastRoundFractionalUp(
          ...
          int scaleDown,
          FastSparkDecimal fastResult) {
    if (scaleDown < 1 || scaleDown > SparkDecimal.MAX_SCALE) {
      throw new IllegalArgumentException("Expecting scaleDown > 0 and scaleDown < " + SparkDecimal.MAX_SCALE + " (scaleDown " + scaleDown + ")");
    }
  ...
  public static boolean fastScaleUp(
          ...
          int scaleUp,
          FastSparkDecimal fastResult) {
    if (scaleUp < 1 || scaleUp >= SparkDecimal.MAX_SCALE) {
  ...
  public static boolean isAllZeroesBelow(
          int fastSignum, long fast0, long fast1, long fast2,
          int power) {
    if (power < 0 || power > SparkDecimal.MAX_SCALE) {
  ...
  public static boolean fastExceedsPrecision(
    ...
    if (precision <= 0) {
      return true;
    } else if (precision >= SparkDecimal.MAX_PRECISION) {
      return false;
    }
    ...
    if (wordNum == 0) {
      ...
      // We've eliminated the highest digit already with SparkDecimal.MAX_PRECISION check above.
  ...
  public static int fastTrailingDecimalZeroCount(
          long fast0, long fast1, long fast2,
          int fastIntegerDigitCount, int fastScale) {
    if (fastScale < 0 || fastScale > SparkDecimal.MAX_SCALE) {
  ...
  private static boolean doFinishAddSubtractDifferentScale(
    ...
    if (precision > SparkDecimal.MAX_PRECISION){
      final int scaleDown = precision - SparkDecimal.MAX_PRECISION;
  ...
  private static boolean doMultiply(
    ...
    if (resultScale < 0) {
      if (-resultScale >= SparkDecimal.MAX_SCALE) {
        return false;
      }
      if (!fastScaleUp(
    ...
    if (precision > SparkDecimal.MAX_PRECISION || resultScale > SparkDecimal.MAX_SCALE) {

      // Trim off lower fractional digits but with NO ROUNDING.

      final int maxScale = SparkDecimal.MAX_SCALE - integerDigitCount;
    ...
    if (fastResult.fastScale > SparkDecimal.MAX_SCALE) {
  ...
  private static boolean fastSingleWordDivision(
    ...
    if (remainderDigitCount == 0) {
      ...
      final int maxScale = SparkDecimal.MAX_SCALE - adjustedQuotientDigitCount;
  ...
  public static boolean fastDivide(
    ...
    */
    ...
    BigDecimal quotient =
            denominator.divide(divisor, SparkDecimal.MAX_SCALE, BigDecimal.ROUND_HALF_UP);
  ...
  public static boolean fastIsValid(FastSparkDecimal fastDec) {
    return fastIsValid(
            fastDec.fastSignum, fastDec.fast0, fastDec.fast1, fastDec.fast2,
            fastDec.fastIntegerDigitCount, fastDec.fastScale);
  }
    ...
    if (fastSignum == 0) {
      ...
      if (!isValid) {
        ...
        if (fastScale < 0 || fastScale > SparkDecimal.MAX_SCALE) {
          ...
          isValid = false;
        } else if (fastIntegerDigitCount < 0 || fastIntegerDigitCount > SparkDecimal.MAX_PRECISION) {
          System.out.println("FAST_IS_VALID fastIntegerDigitCount " + fastIntegerDigitCount + " out of range");
          isValid = false;
        } else if (fastIntegerDigitCount + fastScale > SparkDecimal.MAX_PRECISION) {
