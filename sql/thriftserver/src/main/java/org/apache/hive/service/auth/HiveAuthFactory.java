...
public class HiveAuthFactory {
  ...
  public HiveAuthFactory(HiveConf conf) throws TTransportException, IOException {
    this.conf = conf;
    transportMode = conf.getVar(ConfVars.HIVE_SERVER2_TRANSPORT_MODE);
    authTypeStr = conf.getVar(ConfVars.HIVE_SERVER2_AUTHENTICATION);
      ...
      if (authTypeStr.equalsIgnoreCase(AuthTypes.KERBEROS.getAuthName())) {
        ...
        try {
          ...
          String tokenStoreClass = conf.getVar(
              ConfVars.METASTORE_CLUSTER_DELEGATION_TOKEN_STORE_CLS);
