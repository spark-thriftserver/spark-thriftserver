  ...
  val THRIFTSERVER_CLUSTER_DELEGATION_TOKEN_STORE_CLS =
    buildConf("spark.sql.thriftserver.cluster.delegation.token.store.class")
      ...
      .doc("The delegation token store implementation. " +
        "Set to org.apache.spark.sql.thriftserver.auth.thrift.ZooKeeperTokenStore " +
        "for load-balanced cluster.")
      .stringConf
      .createWithDefault("org.apache.spark.sql.thriftserver.auth.thrift.MemoryTokenStore")
